...
function App() {
  ...
  const validateUserExistence = (res) => {
    ...
    if(flag == 0) {
      alert("User does not exist within database");
    }
  }
    ...
    switch (type) {
      ...
      case "password":
        if (isPasswordInvalid(value)) {
          document.getElementById(em).innerHTML = "Enter Password of 8 characters";
          document.getElementById(em).style.display = "block";
          validPassword = false;
        }
        else if (!value.trim().match(regExPassword1)) {
          document.getElementById(em).innerHTML = "Enter atleast one number";
          document.getElementById(em).style.display = "block";
          validPassword = false;
        }
        else if (!value.trim().match(regExPassword2)) {
          document.getElementById(em).innerHTML = "Enter atleast one special character";
          document.getElementById(em).style.display = "block";
          validPassword = false;
        }
        else {
